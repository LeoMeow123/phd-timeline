...
import type { ItemType, TimelineItem } from '../types';
...
export default function ItemForm({ editItem, onClose }: ItemFormProps) {
  const { tracks, addItem, updateItem, deleteItem, selectItem } = useStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = { name: name.trim(), trackId, start, end: finalEnd, color, type, note: note.trim() || undefined };

    if (isEdit && editItem) {
      updateItem(editItem.id, data);
    } else {
      addItem({ ...data, dependsOn: [] });
    }
    onClose();
  };
